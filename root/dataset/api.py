...
from .vocab import TextVocab, LabelVocab, PosVocab, ChunkVocab
...
def load_dataset():
    ...
    chunk_vocab = ChunkVocab.build(list(map(lambda e: e.chunk, train_examples)))

    train_set = create_dataset(train_examples, text_vocab, labels_vocab, pos_vocab, chunk_vocab)
    val_set = create_dataset(val_examples, text_vocab, labels_vocab, pos_vocab, chunk_vocab)
    test_set = create_dataset(test_examples, text_vocab, labels_vocab, pos_vocab, chunk_vocab)

    return text_vocab, labels_vocab, pos_vocab, chunk_vocab, train_set, val_set, test_set
        ...
        for line in fd:
            ...
            if not line:
                if sentence and labels and pos:
                    examples.append(Example(sentence, labels, pos, chunk))
                    sentence = []
                    labels = []
                    pos = []
                    chunk = []
...
def create_dataset(examples, text_vocab, labels_vocab, pos_vocab, chunk_vocab):
    ...
    chunk = numericalize(chunk_vocab, map(lambda e: e.chunk, examples), pad_token=PAD, maxlen=MAX_LEN)

    return Dataset(X, y, pos, chunk)


Example = namedtuple('Example', 'sentence labels pos chunk')
Dataset = namedtuple('Dataset', 'X y pos chunk')
